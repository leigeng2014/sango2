...
class Redis(object):
    ...
    def zrangebyscore(self, set_name, min_score, max_score):
        """Return all elements in the sorted set ``set_name`` with scores
        between ``min`` and ``max``.
        ...
        :rtype: int
        """      
        return self.conn.zrangebyscore(set_name, min_score, max_score)
